...
show_help() {
  ...
  echo "  all   Dispatch all 4 profiles sequentially (1 every 10 minutes, default)"
}
...
case "$TARGET" in
  1|2|3|4)
    ...
    ;;
  all|"")
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting hisual 4-profile sequence (1 profile every 10 minutes)..."
    for acc in 1 2 3 4; do
    ...
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] All 4 profiles dispatched successfully."
    ;;
  -h|--help|help)
    show_help
    ;;
